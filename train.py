...
def train_one_fold(fold, train_loader, val_loader, config):
    ...
    for epoch in range(config['epochs']):
        model.train()
        train_loop = tqdm(train_loader, desc=f"Fold {fold+1}, Epoch {epoch+1}/{config['epochs']}")
        ...
        if f1 > best_val_f1:
            best_val_f1 = f1
            epochs_no_improve = 0
            model_path = os.path.join(config['save_dir'], f'aeye_best_model_fold_{fold+1}.pth')
            torch.save(model.state_dict(), model_path)
            logging.info(f"New best model saved for Fold {fold+1} with F1-Score: {f1:.4f}")
        else:
            epochs_no_improve += 1
...
def main(config):
    ...
    logging.info(f"Found {len(image_paths)} total images for K-Fold Cross-Validation.")
    ...
    logging.info(f"F1 scores for each fold: {[f'{score:.4f}' for score in fold_scores]}")
    logging.info(f"Average F1-Score: {mean_f1:.4f}")
    logging.info(f"Standard Deviation: {std_f1:.4f}")
...
if __name__ == '__main__':
    ...
    # Model configuration for the 8-RING MODEL
    model_config = {
        'dims': [32, 64, 128, 160],
        'embed_dim': 256,
    }
